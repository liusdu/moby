...
func (s *DockerSuite) TestUpdatePaths(c *check.C) {
	testRequires(c, DaemonIsLinux, NotUserNamespace, NotArm)

	name := "test_update_device_path"
	dockerCmd(c, "run", "-d", "-ti", "--device", "/dev/zero:/dev/exist", "-v", "/tmp:/tmp/exist:rw", "--name", name, "busybox", "top")
	dockerCmd(c, "update", "--add-path", "/tmp:/tmp/test_ok:rw", name)
	res := inspectField(c, name, "HostConfig.Binds")

	expected := "[/tmp:/tmp/exist:rw /tmp:/tmp/test_ok:rw]"
	if res != expected {
		c.Fatalf("Update Path %s, expected %s", res, expected)
	}

	expected = "[{ /tmp /tmp/exist  rw true rprivate} { /tmp /tmp/test_ok  rw true rprivate}]"
	res = inspectField(c, name, "Mounts")
	if res != expected {
		c.Fatalf("Update Path %s, expected %s", res, expected)
	}

	dockerCmd(c, "update", "--remove-path", "/tmp:/tmp/test_ok:rw", name)
	res = inspectField(c, name, "HostConfig.Binds")
	expected = "[/tmp:/tmp/exist:rw]"
	if res != expected {
		c.Fatalf("Update Path %s, expected %s", res, expected)
	}

	expected = "[{ /tmp /tmp/exist  rw true rprivate}]"
	res = inspectField(c, name, "Mounts")
	if res != expected {
		c.Fatalf("Update Path %s, expected %s", res, expected)
	}

	dockerCmd(c, "update", "--add-device", "/dev/zero:/dev/test_device:rw", name)
	res = inspectField(c, name, "HostConfig.Resources.Devices")
	expected = "[{/dev/zero /dev/exist rwm} {/dev/zero /dev/test_device rw}]"
	if res != expected {
		c.Fatalf("Update Device %s, expected %s", res, expected)
	}

	dockerCmd(c, "update", "--remove-device", "/dev/zero:/dev/test_device:rw", name)
	res = inspectField(c, name, "HostConfig.Resources.Devices")
	expected = "[{/dev/zero /dev/exist rwm}]"
	if res != expected {
		c.Fatalf("Update Device %s, expected %s", res, expected)
	}
