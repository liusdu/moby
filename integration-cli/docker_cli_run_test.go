...
func (s *DockerSuite) TestRunDnsOptions(c *check.C) {
	...
	out, _, _ = dockerCmdWithStdoutStderr(c, "run", "--dns=127.0.0.1", "--dns-search=.", "--dns-opt=ndots:3", "busybox", "cat", "/etc/resolv.conf")
	...
	hookStr := `
{
	...
	dockerCmd(c, "run", "-v", homePath+":/someplace", "--hook-spec="+hookSpecFile, "busybox", "ls", filepath.Join("/someplace", testDir))
...
func (s *DockerSuite) TestRunWithInvalidHookSpecPath(c *check.C) {
	testRequires(c, DaemonIsLinux)
	homePath := randomTmpDirPath("test1", daemonPlatform)
	testDir := "testHook"
	...
	hookStr := `
{
	...
	// hook path is not subdir of "/var/lib/docker/hooks", so this `docker run` is
	// expected to fail and should return error
	_, _, err = dockerCmdWithError("run", "-v", homePath+":/someplace", "--hook-spec="+hookSpecFile, "busybox", "ls", filepath.Join("/someplace", testDir))
	c.Assert(err, checker.NotNil)
}
...
func (s *DockerSuite) TestRunAddCapWithSystemContainer(c *check.C) {
	...
	out, _, err := runCommandWithOutput(runCmd)
	c.Assert(err, checker.IsNil)
	c.Assert(strings.TrimSpace(out), checker.Equals, "CapEff:	00000000e84425fb")
...
func (s *DockerSuite) TestRunUnblockSyscallUnshareWithSystemContainer(c *check.C) {
	...
	out, _, err := runCommandWithOutput(runCmd)
	c.Assert(err, checker.IsNil)
	c.Assert(strings.TrimSpace(out), checker.Equals, "root")
}
