...
elif [ $(cat COMMIT)x != x ]; then
	GITCOMMIT="$(cat COMMIT)"
else
...
ORIG_BUILDFLAGS=( -tags "autogen netgo static_build sqlite_omit_load_extension libsqlite3 $DOCKER_BUILDTAGS" -installsuffix netgo )
...
test_env() {
	...
	env -i \
		...
		DOCKER_STORAGE_OPTS2="$DOCKER_STORAGE_OPTS2" \
